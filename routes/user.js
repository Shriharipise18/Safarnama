...
router.get('/auth/github', (req, res, next) => {
    ...
    if (!process.env.GITHUB_CLIENT_ID || !process.env.GITHUB_CLIENT_SECRET) {
        console.error('GitHub OAuth not properly configured. Missing credentials.');
        return res.render('signin', {
            error: "GitHub authentication is not available. Please sign in with email and password."
        });
    }
...
router.get('/auth/github/callback', (req, res, next) => {
    ...
    if (!process.env.GITHUB_CLIENT_ID || !process.env.GITHUB_CLIENT_SECRET) {
        console.error('GitHub OAuth not properly configured. Missing credentials.');
        return res.redirect('/user/signin?error=github_auth_unavailable');
    }
